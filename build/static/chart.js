 ...
 * @async
 * @returns {Promise<Object>} Промис с данными в формате JSON
 */
...
/**
 * Функция инициализации диаграммы с использованием данных
 * @param {Array} dataFlattened - Данные для диаграммы
 */
 ...
 * @param {Event} e - Событие ввода в строку поиска
 */
 ...
 * @param {HTMLElement} elem - Элемент, который переходит в полноэкранный режим
 */
 ...
 * @returns {d3.OrgChart} Возвращает объект диаграммы с обновлениями
 */
...
function expandAllCustom() {
    ...
    // allNodes.forEach(d => chart.expand(d));
    chart.render().fit();
    // Определяем максимальную глубину узлов
    const maxDepth = Math.max(...allNodes.map((node) => node.depth));
    if (allNodes.length > 0) {
